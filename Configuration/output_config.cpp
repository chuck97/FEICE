...
OutputParams::OutputParams(const std::string& json_path)
{
    ...
    // Parse output gap
    if (j_input.count("number of screnshots") != 0)
    {
        number_of_screenshots = j_input["number of screnshots"];
    }
    BARRIER
...
int OutputParams::GetNumberOfScreenshots() const
{
...
{
    cout << endl;
    cout << "#### Output config params ####" << endl;
    if (is_pvtu_output)
    {
    ...
    cout << "number of screenshots: " << number_of_screenshots << endl;
