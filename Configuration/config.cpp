...
ConfigParams::ConfigParams(const string& json_path)
{
    ...
    // Parse dynamics test params .json path if not empty
    ...
    // Parse ice forcing .json path if not empty
    if (!j_input["ice forcing params"].empty())
    {
        ice_forcing_params = IceForcingParams(j_input["ice forcing params"]);
    }
    BARRIER

    // Parse water forcing .json path if not empty
    if (!j_input["water forcing params"].empty())
    {
        water_forcing_params = WaterForcingParams(j_input["water forcing params"]);
    }
    BARRIER

    // Parse air forcing .json path if not empty
    if (!j_input["air forcing params"].empty())
    {
        air_forcing_params = AirForcingParams(j_input["air forcing params"]);
    }
    BARRIER
};
...
IceForcingParams& ConfigParams::GetIceForcingParams()
{
    return ice_forcing_params;
};

WaterForcingParams& ConfigParams::GetWaterForcingParams()
{
    return water_forcing_params;
};

AirForcingParams& ConfigParams::GetAirForcingParams()
{
    return air_forcing_params;
};